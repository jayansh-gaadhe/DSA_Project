...
class LL{//linked list->to store not repeating coursedata
public:
    string pg;
    int sem;
    string fac;
...
int main()
{
    ...
    cin >> fname;
    in.open(fname);//for counting 
    in2.open(fname);//for reading
    while (in.peek() != EOF)
    {
        getline(in, content);
        count++;
    }
    cout << "Count is : " << count << endl;//count=Total number of course
    Course arr[count];
    for (int i = 0; i < count; i++)
    {
        arr[i].read_data();
    ...
    for (int i = 0; i < count; i++)
    {
        if (repeat_course(t, arr[i].get_code(), temp) == 0)
        {
